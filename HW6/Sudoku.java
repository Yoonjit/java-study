import java.util.Random; // 모델 (미완성)

public class Sudoku {

    private int[][] solution = new int[9][9]; // 스도쿠 정답
    private int hole_count; // 채워야 할 칸의 수
    private int[][] puzzle_board = new int[9][9]; // 스도쿠 판
    ...
    public Sudoku(int count) {
        createSolutionBoard(); // 스도쿠 정답 생성하는 객체
        hole_count = count; // 채워야 할 칸의 수 = count
        createPuzzleBoard(count); // 빈칸 수 만큼 뚫어놓고 스도쿠 생성 
    }
